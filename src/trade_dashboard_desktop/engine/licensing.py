...
key validated against a merchant of record (Gumroad / Lemon Squeezy) without
touching the UI or engine code.

Key format: ``TD-XXXX-XXXX-XXXX`` (uppercase alphanumerics).  Keys are stored
in ``~/.trade_dashboard_desktop/license.key`` and never leave the machine in
v0.1.0 (offline format check only).
"""
...
KEY_PATTERN = re.compile(r"^TD-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}$")
CONFIG_DIR = Path(os.path.expanduser("~")) / ".trade_dashboard_desktop"
